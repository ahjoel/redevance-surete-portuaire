...
type
  TFTypeFacture = class(TUniForm)
    ...
    procedure BtnCancelCFClick(Sender: TObject);
    procedure BtnCancelTFClick(Sender: TObject);
  private
                      ...
                      case Res of
                        mrYes :
                            begin
                                  with DM.QUpdate do
                                        begin
                                            SQL.Text := ('UPDATE categorie_fact SET libelle_cat_fact=:libelle, code_cat_fact=:code WHERE id_cat=:id ');
                                            Parameters.ParamByName('id').Value:= id_CatFact;
                                            Parameters.ParamByName('libelle').Value:= EdLibCat.Text;
                                            Parameters.ParamByName('code').Value:= EdCodeCat.Text;
                                            ExecSQL;
                                        end;
...
procedure TFTypeFacture.BtnCancelCFClick(Sender: TObject);
begin
    FmContextCF := AddContext;
    ClearDataCF;
    EdLibCat.JSInterface.JSCall('focus' ,[]);
...
procedure TFTypeFacture.BtnCancelTFClick(Sender: TObject);
begin
    FmContextTF := AddContext;
    ClearDataTF;
    EdLibelle.JSInterface.JSCall('focus' ,[]);
