    ...
    //                    TfrxGroupHeader(FEtatsEscale.frxReport_group.FindObject('GroupHeader')).Condition :=  'frxDBDataset."E.consignataire"'  ;
                 ...
                 case CBGroup.ItemIndex of
                      ...
                      3 : begin
                            ...
                            filter_group := 'GROUP BY Q.ip_pq ' ;
                            order_group :=  ' ORDER BY Q.ip_pq '  ;
                            title_group := '[frxDBDataset."libelle_ip"]';
                          end;

                      4 : begin
                            data_group := 'frxDBDataset."libelle_pq"';
                            filter_group := ' GROUP BY E.post_quai ' ;
                            order_group :=  ' ORDER BY E.post_quai '  ;
                            title_group := '[frxDBDataset."libelle_pq"]';
                          end;

                      5 : begin
                            data_group := 'frxDBDataset."libelle_obs"';
                            filter_group := ' GROUP BY E.observation ' ;
                            order_group :=  ' ORDER BY E.observation '  ;
                            title_group := '[frxDBDataset."libelle_obs"]';
                          end;

                      6 : begin
                            data_group := 'frxDBDataset."statut"'   ;
                            filter_group := ' GROUP BY statut ' ;
                            order_group :=  ' ORDER BY statut '  ;
                            title_group := '[frxDBDataset."statut"]';
                          end;
