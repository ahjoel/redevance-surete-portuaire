...
begin
  ...
  DBLDataLoad(DM.DQ_DBL_Cons_fact,'SELECT * FROM consignataire WHERE id_consignataire is not null order by nom_consignataire');
end;
...
procedure LoadDBLTypeFactRade;
begin
    DBLDataLoad(DM.DQ_DBL_TypeFact,'SELECT  * FROM type_facture T, taux_rade R WHERE T.id_type_fact IN (SELECT T.id_type_fact WHERE R.type_fact=T.id_type_fact )');
end;

procedure LoadDBLCatFact;
begin
    DBLDataLoad(DM.DQ_DBL_Cat_Fact,'SELECT * FROM categorie_fact WHERE id_cat is not null');
end;
