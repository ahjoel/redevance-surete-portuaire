  ...
  if DBLProfilAttrib.KeyValue = null then
    ...
    with  DM.QCheck do
      ...
      if DM.QCheck.RecordCount > 0 then
           begin
               MessageDlg('Un Permission deja attribuée a ce Profil !',mtError,[mbok]);
               DBGridListPerms.JSInterface.JSCall('focus' ,[]);
               Abort;
           end
      else
...
begin
      MessageDlg('Voulez-vous Attriber cette permission á ce Profile ?', mtConfirmation, mbYesNo,
      ...
      begin
        ...
        if EdLibPerms.IsBlank then
            ...
            with  DM.QCheck do
              ...
              if DM.QCheck.RecordCount > 0 then
                   begin
                       MessageDlg('Une Permissions posséde dejà ce Libellé',mtError,[mbok]);
                       EdLibPerms.JSInterface.JSCall('focus' ,[]);
                       Abort;
                   end
              else
          ...
          if EdCodePerms.IsBlank then
            ...
            with  DM.QCheck do
              ...
              if DM.QCheck.RecordCount > 0 then
                   begin
                       MessageDlg('Une Permissions posséde dejà ce Code',mtError,[mbok]);
                       EdCodePerms.JSInterface.JSCall('focus' ,[]);
                       Abort;
                   end
              else
      ...
      begin
          ...
          if EdLibProfil.IsBlank then
            ...
            with  DM.QCheck do
              ...
              if DM.QCheck.RecordCount > 0 then
                   begin
                       MessageDlg('Un Libelle de Profil posséde dejà ce Libellé',mtError,[mbok]);
                       EdLibProfil.JSInterface.JSCall('focus' ,[]);
                       Abort;
                   end
              else
      ...
      if (EdLibProfil.Text <> lib_Profil) then
           begin
               with  DM.QCheck do
                  ...
                  if DM.QCheck.RecordCount > 0 then
                       begin
                           MessageDlg('Un Profil posséde dejà ce Libellé',mtError,[mbok]);
                           EdLibProfil.JSInterface.JSCall('focus' ,[]);
                           Abort;
                       end
           end ;
  ...
  if FmContextUser = AddContext then
      begin
        ...
        if EdFirstNameUser.IsBlank then
            begin
               MessageDlg('Veuillez renseigner le Prénoms',mtWarning,[mbok]);
            ...
            with  DM.QCheck do
              ...
              if DM.QCheck.RecordCount > 0 then
                   begin
                       MessageDlg('Un Utilisateur posséde dejà cette Identité',mtError,[mbok]);
                       EdLastNameUser.JSInterface.JSCall('focus' ,[]);
                       Abort;
                   end
              else
            ...
            begin
              ...
              if (EdLoginUser.Text <> login_User) then
                   begin
                       with  DM.QCheck do
                          ...
                          if DM.QCheck.RecordCount > 0 then
                               begin
                                   MessageDlg('Un Utilisateur posséde dejà ce Login ',mtError,[mbok]);
                                   EdLoginUser.JSInterface.JSCall('focus' ,[]);
                                   Abort;
                               end
                   end ;
              ...
              if DBLProfilUser.IsBlank then
                  begin
                     MessageDlg('Veuillez sélectionner  le Profil Utilisateur ',mtWarning,[mbok]);
 ...
 if FmContextUser = UpdateContext then
    begin
      if EdLastNameUser.IsBlank then
       ...
       if EdFirstNameUser.IsBlank then
            begin
               MessageDlg('Veuillez renseigner le Prénom',mtWarning,[mbok]);
               EdFirstNameUser.JSInterface.JSCall('focus' ,[]);
               Abort;
            end ;
