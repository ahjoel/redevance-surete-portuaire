          ...
          if EdLibelle.IsBlank then
            ...
            with  DM.QCheck do
              begin
                   ...
                   Parameters.ParamByName('libelle_exercice').Value:=EdLibelle.text;
                   ExecSQL;
                   Open;
              end;
              if DM.QCheck.RecordCount > 0 then
                   begin
                       MessageDlg('Un Exercice posséde dejà ce Libellé',mtError,[mbok]);
                       EdLibelle.JSInterface.JSCall('focus' ,[]);
                       Abort;
                   end
              else
                                  ...
                                  begin
                                        with DM.QSave do
                                              begin
                                                  ...
                                                  Parameters.ParamByName('actif').Value:= ABS(RadActif.ItemIndex-1);
                                                  ExecSQL;
                                              end;
                                             ClearData;
      ...
      if (EdLibelle.Text <> lib_Exo) then
           begin
               with  DM.QCheck do
                  ...
                  if DM.QCheck.RecordCount > 0 then
                       begin
                           MessageDlg('Un Exercice dejà ce Libellé',mtError,[mbok]);
                           EdLibelle.JSInterface.JSCall('focus' ,[]);
                           Abort;
                       end
           end ;
                            ...
                            begin
                                  with DM.QUpdate do
                                        begin
                                            ...
                                            Parameters.ParamByName('id').Value:= id_Exo;
                                            Parameters.ParamByName('libelle').Value:= EdLibelle.Text;
                                            Parameters.ParamByName('actif').Value:= ABS(RadActif.ItemIndex-1);
                                            ExecSQL;
                                        end;
...
begin
 ...
 with DM.QStand do
           ...
           Parameters.ParamByName('id').Value:= DBGrid.DataSource.DataSet.FieldByName('id_exercice').AsString;
           ExecSQL;
           Open;
           end;
           ...
           begin
               ...
               RadActif.ItemIndex :=1-(DM.QStand.FieldValues['actif_exercice']);
           end
end;
...
procedure TFExercice.ShowData;
  begin
      filter:=' order by libelle_exercice DESC';
